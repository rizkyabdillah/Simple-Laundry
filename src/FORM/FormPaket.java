...
public class FormPaket extends javax.swing.JDialog{
    ...
    private final String header[] = {
        "Id Paket", "Nama Paket", "Harga Per Kilo", "Deskripsi Paket"
    };

    public FormPaket(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        showTabel("");
    }
        ...
        try {
            String sql = "SELECT * FROM PAKET WHERE "
                    + "id_paket LIKE '%" + key + "%' OR "
                        + "nama_paket LIKE '%" + key + "%' OR "
                            + "harga_perkg LIKE '%" + key + "%' OR "
                                + "deskripsi_paket LIKE '%" + key + "%'";
            ...
            tabel.getColumnModel().getColumn(3).setMinWidth(350);
        } catch (Exception e) {
    ...
    private void initComponents() {
        ...
        jLabel1.setText("FORM KELOLA PAKET");
    ...
    private void onTambah(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onTambah
        ...
        new DialogAddPaket(null, true).setVisible(true);
        showTabel("");
        id = "";
    ...
    private void onHapus(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_onHapus
        if(!id.equals("")) {
            int i = dialog.showConfirm("Apakah anda yakin ingin menghapus paket bernama " + nama + " ?");
            if(i == JOptionPane.YES_OPTION) {
                con.setExecute("DELETE FROM PAKET WHERE id_paket ='" + id + "'");
            }
        } else {
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(() -> {
            FormPaket dialog = new FormPaket(new javax.swing.JFrame(), true);
